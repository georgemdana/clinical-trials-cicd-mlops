# code to run model to find patients within age bands
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from snowflake_queries import connect_to_snowflake, get_patient_data, get_age_feature_data, find_patients_age, democount, modelcount
...
def age_model():
    ...
    patient_age1 = execute_sql(conn, patient_age_query)
    ...
    trial_age = execute_sql(conn, trial_age_query)
    clinical_trial_age = pd.DataFrame(trial_age)
    clinical_trial_age.columns = ['TRIAL_ID', 'INCLUSION_CRITERIA', 'AGES', 'MIN_AGE', 'MAX_AGE']

    # Create all possible patient-trial combinations
    combinations = pd.merge(patient_age, clinical_trial_age, how="cross")
#    combinations = patient_age.assign(key=1).merge(clinical_trial_age.assign(key=1), on='key', how='outer').drop('key', axis=1)
    print(combinations)
