...
def get_trial_data(conn):
    """
    Fetches trial data from Snowflake.
    """
    query_trial = "SELECT * FROM MITSUI_DEV.DATASCIENCE_DEV.CLINICAL_TRIALS"
    return query_data(conn, query_trial)

def get_patient_data(conn):
    """
    Fetches demographic data from Snowflake.
    """
    query_patient = "SELECT * FROM MITSUI_DEV.DATASCIENCE_DEV.PATIENT_DATA"
    return query_data(conn, query_patient)
